...
func (b *Bson) AppendInt32(key string, val int32) bool {
	return bool(C.bson_append_int32((*C.bson_t)(b), C.CString(key), C.int(len(key)), C.int32_t(val)))
}
...
func (b *Bson) AppendInt64(key string, val int64) bool {
	return bool(C.bson_append_int64((*C.bson_t)(b), C.CString(key), C.int(len(key)), C.int64_t(val)))
}
...
func (b *Bson) AppendMaxKey(key string) bool {
	return bool(C.bson_append_maxkey((*C.bson_t)(b), C.CString(key), C.int(len(key))))
}
...
func (b *Bson) AppendMinKey(key string) bool {
	return bool(C.bson_append_minkey((*C.bson_t)(b), C.CString(key), C.int(len(key))))
}
...
func (b *Bson) AppendNowUTC(key string) bool {
	return bool(C.bson_append_now_utc((*C.bson_t)(b), C.CString(key), C.int(len(key))))
}
...
func (b *Bson) AppendDateTime(key string, timestamp int64) bool {
	return bool(C.bson_append_date_time((*C.bson_t)(b), C.CString(key), C.int(len(key)), C.int64_t(timestamp)))
}
